...
def zgload_data(model, ensN):
  pwd = '/work2/CMIP6/CMIP6_monthly/ZG/HIST_ALL/' 
  zgloadfiles = []
  for run in range(1,ensN+1):
    ...
    dataF = pwd + 'zg_Amon_' + model + '_historical_' + member + '*' + '.nc'         
...
#ens = [10, 3, 5]
#fname = ['IPSL-CM6A-LR','MIROC6', 'MRI-ESM2-0']
...
for idx, f in enumerate(fname):
  for mem in range(ens[idx]): 
    ...
    ndj = np.zeros((54, len(lat), len(lon)))
    zg_arr = np.array(zgds_re)    

    for year in range(1, 55):
...
zgmodelArr = np.save('zg_all.npy', zgmodelArr)
zgmasterArr = np.save('zg_all_model.npy', zgmasterArr)
...
for idx, f in enumerate(fname):
  for mem in range(ens[idx]):
    ...
    #print('shape of tasds_re =', np.shape(tasds_re))
    #print(tasds_re['lon'])
    #exit()

    #plt.contourf(tasds_re[0])
    #plt.show()
    ...
    lat = tasds_re.coords['lat']
    lon = tasds_re.coords['lon']
    time = tasds_re.coords['time']
...
tasmodelArr = np.save('tas_all.npy', tasmodelArr)
tasmasterArr = np.save('tas_all_model.npy', tasmasterArr)
'''
# --------------------------  
...
ninomodelArr = np.save('nino_all.npy', ninomodelArr)
ninomasterArr = np.save('nino_all_model.npy', ninomasterArr)
'''
exit()
