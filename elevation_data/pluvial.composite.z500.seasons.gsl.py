...
for idx in range(len(p_all)):
    ...
    # ----------------------
    period1 = p_all[idx].sel(time = slice('1950','1979')).mean('time')
    ...
    # ----------------------
    period2 = p_all[idx].sel(time = slice('1980','2021')).mean('time')
...
ax[0,0].set_title('JFM (PLUVIAL)', loc = 'left')
ax[0,1].set_title('JJA (PLUVIAL)', loc = 'left')
ax[0,2].set_title('Annual (PLUVIAL)', loc = 'left')
...
for row in range(3):
    cbar = fig.colorbar(cplots[row], ax = ax[row, :], 
                         shrink = 0.5, 
...
#plt.tight_layout()
#plt.savefig('pluvial.z500.composites.png', dpi = 500)
#fig.suptitle('Pluvial Years 500hpa anomaly composites')
