...
# ----------------------------------------------
# LOAD DATA: Observational T2M data from ERA5
# -----------------------------------------------

pwd_obs = '/work2/Reanalysis/ERA5/ERA5_monthly/monolevel/ERA5.mon.t2m.1950-2020.nc'
ds = xr.open_dataset(pwd_obs)#.t2m#.sel(time=slice("1960-01-01","2014-12-01"),
	#longitude=slice(110,250), latitude=slice(45, -45))#['t2m']

tvar = ds.sel(time=slice("1960-01-01","2014-12-01"))['t2m'] - 273.15
...
t2m = t.sel(lon=slice(110,250), lat=slice(-45, 45))
nino = t.sel(lon=slice(190,240), lat=slice(-5, 5))
...
lat = nino.coords['lat']
lon = nino.coords['lon']
nino_arr = np.array(nino) #convert from K to C
...
s = np.save('SST_obs.npy', SST_obs)
print(np.shape(SST_obs))
exit()
